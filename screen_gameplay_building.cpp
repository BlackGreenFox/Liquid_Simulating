...
void FluidCubeAddDensity(FluidCube& cube, int x, int y, float amount) {
    if (x >= 0 && x < N && y >= 0 && y < N && !cube.wall[IX(x, y)]) {
...
static void set_bnd(int b, std::vector<float>& x) {
    for (int i = 1; i < N - 1; i++) {
        if (IX(i, 0) < x.size() && IX(i, 1) < x.size() && IX(i, N - 1) < x.size() && IX(i, N - 2) < x.size()) {
            if (!cube.wall[IX(i, 0)]) x[IX(i, 0)] = b == 2 ? -x[IX(i, 1)] : x[IX(i, 1)];
            if (!cube.wall[IX(i, N - 1)]) x[IX(i, N - 1)] = b == 2 ? -x[IX(i, N - 2)] : x[IX(i, N - 2)];
        }
    }
    for (int j = 1; j < N - 1; j++) {
        if (IX(0, j) < x.size() && IX(1, j) < x.size() && IX(N - 1, j) < x.size() && IX(N - 2, j) < x.size()) {
            if (!cube.wall[IX(0, j)]) x[IX(0, j)] = b == 1 ? -x[IX(1, j)] : x[IX(1, j)];
            if (!cube.wall[IX(N - 1, j)]) x[IX(N - 1, j)] = b == 1 ? -x[IX(N - 2, j)] : x[IX(N - 2, j)];
        }
    }
    if (IX(0, 0) < x.size() && IX(1, 0) < x.size() && IX(0, 1) < x.size()) {
        if (!cube.wall[IX(0, 0)]) x[IX(0, 0)] = 0.5f * (x[IX(1, 0)] + x[IX(0, 1)]);
    }
    if (IX(0, N - 1) < x.size() && IX(1, N - 1) < x.size() && IX(0, N - 2) < x.size()) {
        if (!cube.wall[IX(0, N - 1)]) x[IX(0, N - 1)] = 0.5f * (x[IX(1, N - 1)] + x[IX(0, N - 2)]);
    }
    if (IX(N - 1, 0) < x.size() && IX(N - 2, 0) < x.size() && IX(N - 1, 1) < x.size()) {
        if (!cube.wall[IX(N - 1, 0)]) x[IX(N - 1, 0)] = 0.5f * (x[IX(N - 2, 0)] + x[IX(N - 1, 1)]);
    }
    if (IX(N - 1, N - 1) < x.size() && IX(N - 2, N - 1) < x.size() && IX(N - 1, N - 2) < x.size()) {
        if (!cube.wall[IX(N - 1, N - 1)]) x[IX(N - 1, N - 1)] = 0.5f * (x[IX(N - 2, N - 1)] + x[IX(N - 1, N - 2)]);
    }
}
...
static void lin_solve(int b, std::vector<float>& x, std::vector<float>& x0, float a, float c, int iter) {
    ...
    for (int k = 0; k < iter; k++) {
        for (int j = 1; j < N - 1; j++) {
            for (int i = 1; i < N - 1; i++) {
                if (IX(i, j) < x.size() && IX(i + 1, j) < x.size() && IX(i - 1, j) < x.size() && IX(i, j + 1) < x.size() && IX(i, j - 1) < x.size() && !cube.wall[IX(i, j)]) {
...
static void project(std::vector<float>& velocX, std::vector<float>& velocY, std::vector<float>& p, std::vector<float>& div, int iter) {
    for (int j = 1; j < N - 1; j++) {
        for (int i = 1; i < N - 1; i++) {
            if (IX(i, j) < velocX.size() && IX(i + 1, j) < velocX.size() && IX(i - 1, j) < velocX.size() && IX(i, j + 1) < velocY.size() && IX(i, j - 1) < velocY.size() && !cube.wall[IX(i, j)]) {
    ...
    for (int j = 1; j < N - 1; j++) {
        for (int i = 1; i < N - 1; i++) {
            if (IX(i, j) < velocX.size() && IX(i + 1, j) < p.size() && IX(i - 1, j) < p.size() && IX(i, j + 1) < p.size() && IX(i, j - 1) < p.size() && !cube.wall[IX(i, j)]) {
...
static void advect(int b, std::vector<float>& d, std::vector<float>& d0, std::vector<float>& velocX, std::vector<float>& velocY, float dt) {
    ...
    for (int j = 1; j < N - 1; j++) {
        for (int i = 1; i < N - 1; i++) {
            if (IX(i, j) < d.size() && IX(i, j) < velocX.size() && IX(i, j) < velocY.size() && !cube.wall[IX(i, j)]) {
...
void CreateWalls() {
    // Create a rectangle in the middle (20x30 size)
    int startX = (N - 30) / 2;
    int startY = (N - 20) / 2;
    int endX = startX + 30;
    int endY = startY + 20;
    ...
    for (int y = startY; y < endY; y++) {
        cube.wall[IX(startX, y)] = true;       // Left side of rectangle
        cube.wall[IX(endX - 1, y)] = true;     // Right side of rectangle
    }
    for (int x = startX; x < endX; x++) {
        cube.wall[IX(x, startY)] = true;       // Top side of rectangle
        cube.wall[IX(x, endY - 1)] = true;     // Bottom side of rectangle
    }
}
    ...
    for (int y = 0; y < N; y++) {
        for (int x = 0; x < N; x++) {
            if (IX(x, y) < cube.density.size()) {
                ...
                Color color2 = { colorValue, colorValue, colorValue, 255 };
                Color color = cube.wall[IX(x, y)] ? RED : color2;
